...
/**
 * Returns  vite / esbuild plug-in.
 */
export default function dyadTagger() {
  return {
    ...
    async transform(code: string, id: string) {
      console.log("transform", id, "code.length", code.length);
      ...
      walk(ast as any, {
        enter(node: any) {
          ...
          const alreadyTagged = node.attributes.some(
            (attr: any) =>
              attr.type === "JSXAttribute" && attr.name.name === "data-dyad-id",
          );
          if (alreadyTagged) return;

          // ── 3. Build the id “relative/file.jsx:line:column” ─────────────────
          const loc = node.loc!.start; // guaranteed when parser is given source
